...
function systemPrompt(mode: PlanMode): string {
  ...
  return [
    ...
    '',
    'The renderer refuses prompts that breach its content policy, and a refused shot still',
    'costs a slot and the time to find out. Write prompts that will actually render:',
    '- Invent original people. Never name or describe a real, identifiable person — no',
    '  public figures, celebrities, politicians or anyone recognisable as a specific human.',
    '- No brands, logos, trademarks, or characters from existing films, games or comics.',
    '- No blood, wounds, gore, corpses, or violence being inflicted. Convey danger through',
    '  aftermath, reaction, tension, shadow and sound-free implication instead of depiction.',
    '- Nothing sexual or suggestive. Describe clothing plainly and keep framing non-intimate.',
    '- No weapons pointed at people, no self-harm, no drug use, no hateful or extremist',
    '  imagery or symbols.',
    '- Take particular care with children: if a minor appears, keep them clothed, safe,',
    '  ordinary and incidental to the shot.',
    '- Avoid words that read as violent out of context. Prefer "camera moves in" to',
    '  "shooting", "push in" to "trigger", and describe a "still figure" rather than a body.',
    '- Do not depict real events, disasters, or anything that could pass as real footage of',
    '  something that happened.',
    mode === 'story'
      ? '- If the source contains material the renderer would refuse, keep the beat but stage it safely: cut away, imply it, or show the moment after. Never drop a beat, and never make it graphic.'
      : '- Where a brief hints at something the renderer would refuse, reach the same emotional effect by implication rather than by showing it.',
    '',
